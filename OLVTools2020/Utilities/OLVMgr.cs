...
namespace OLVTools2020
{
    ...
    public class OLVMgr
    {
        public OLVMgr(Form oParentForm)
        {
            parentForm = oParentForm;
        }
        ...
        public string ToolStripStatus1 {
            get { return Tool_Strip_StatusLabel.Text; }
            set {
                Tool_Strip_StatusLabel.Text = value;
                this.Parent_Form.Update();
            }
        }

        public ToolStripLabel Tool_Strip_StatusLabel = new ToolStripLabel();
        ...
        public void HandleHotItemChanged(object sender, HotItemChangedEventArgs e)
        {
            ...
            switch (e.HotCellHitLocation)
            {
                case HitTestLocation.Nothing:
                    Tool_Strip_StatusLabel.Text = @"Over nothing";
                    break;
                case HitTestLocation.Header:
                case HitTestLocation.HeaderCheckBox:
                case HitTestLocation.HeaderDivider:
                    Tool_Strip_StatusLabel.Text = String.Format("Over {0} of column #{1}", e.HotCellHitLocation, e.HotColumnIndex);
                    break;
                case HitTestLocation.Group:
                    Tool_Strip_StatusLabel.Text = String.Format("Over group '{0}', {1}", e.HotGroup.Header, e.HotCellHitLocationEx);
                    break;
                case HitTestLocation.GroupExpander:
                    Tool_Strip_StatusLabel.Text = String.Format("Over group expander of '{0}'", e.HotGroup.Header);
                    break;
                default:
                    Tool_Strip_StatusLabel.Text = String.Format("Over {0} of ({1}, {2})", e.HotCellHitLocation, e.HotRowIndex, e.HotColumnIndex);
                    break;
            }
        }
